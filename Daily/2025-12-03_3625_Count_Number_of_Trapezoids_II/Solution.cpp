...
public:
  ...
  static int pack2(int a, int b) { return ((ll)(a + bias) << 16) | (b + bias); }
  ...
  static ll pack4(int a, int b, int c, int d) {
    return ((ll)(a + bias) << 48) | ((ll)(b + bias) << 32) |
           ((ll)(c + bias) << 16) | (d + bias);
  }
  ...
  static int countTrapezoids(vector<vector<int>> &points) {
    ...
    for (int i = 0; i < n - 1; i++) {
      ...
      for (int j = i + 1; j < n; j++) {
        ...
        int gm = __gcd(a, b), gc = __gcd(gm, abs(c));
        ...
        ll midab = pack4(x0 + x1, y0 + y1, a / gm, b / gm); // midpoint+slope
                                                            // key

        cnt += (slope[ab]++) - (coeff[abc]++) -
